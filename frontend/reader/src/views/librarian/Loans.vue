...
async function rejectBorrow(item) {
  const reason = libStore.promptRejectReason('Không đủ điều kiện mượn sách')
  if (!reason) return
  actionId.value = actionKey(item, 'reject')
  try {
    await libStore.rejectTransaction(item.Id || item.id, reason)
  } finally {
    actionId.value = null
  }
}
...
async function renew(item) {
  const reason = libStore.promptRejectReason('Gia hạn theo đề nghị độc giả')
  if (!reason) return
  actionId.value = actionKey(item, 'renew')
  try {
    await libStore.renewTransaction(item.Id || item.id, 7, reason)
  } finally {
    actionId.value = null
  }
}
...
async function rejectRenewal(item) {
  const reason = libStore.promptRejectReason('Không đủ điều kiện gia hạn')
  if (!reason) return
  actionId.value = actionKey(item, 'reject-renew')
  try {
    await libStore.rejectRenew(item.Id || item.id, reason)
  } finally {
    actionId.value = null
  }
}
...
async function rejectReturn(item) {
  const reason = libStore.promptRejectReason('Không đủ điều kiện trả sách')
  if (!reason) return
  actionId.value = actionKey(item, 'return-reject')
  try {
    await libStore.rejectReturn(item.Id || item.id, reason)
  } finally {
    actionId.value = null
  }
}
